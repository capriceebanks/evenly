...
import { updateEvent } from '@/lib/actions/user.actions';
...
const EventForm = ({ userId, type, event, eventId }: EventFormProps) => {
	...
	const onSubmit = async (values: z.infer<typeof eventFormSchema>) => {
		...
		if (files.length > 0) {
			// If there is a file present, change image to url and
		...
		if (type === 'Create') {
			try {
				...
				if (newEvent) {
					//If there's a new event, reset form and go to new event page
					form.reset();
		...
		if (type === 'Update') {
			...
			try {
				...
				if (updatedEvent) {
					//If the event has been updated, go to event page
					form.reset();
